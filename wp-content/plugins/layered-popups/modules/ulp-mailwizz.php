...
<body>';
							if (array_key_exists('status', $lists) && $lists['status'] == 'error') {
								echo '<div style="text-align: center; margin: 20px 0px;">'.esc_html($lists['error']).'</div>';
							} else {
								echo '
	<table style="width: 100%;">
		...
		<tr>
			...
			<td>'.esc_html(esc_html($value)).'</td>
						...
						} else echo '<div style="text-align: center; margin: 20px 0px;">'.__('Invalid server response!', 'ulp').'</div>';
...
//		if ($httpCode != '200') return array();
		...
		$result = json_decode($response, true);
		if(!$result || !is_array($result)) return array('status' => 'error', 'error' => 'Invalid server response.');
